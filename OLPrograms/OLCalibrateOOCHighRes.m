function OLCalibrateOOCHighRes
...
try
    ...
    if (cal.describe.doGamma)
        ...
        % the Gamma levels to be measured
        cal.describe.gamma.gammaLevels = linspace(1/cal.describe.nGammaLevels,1,cal.describe.nGammaLevels);
        
        % The Gamma bands to be measured
        if (ischar(cal.describe.nGammaBands))
            switch cal.describe.nGammaBands
                case 'everyOtherPrimary'
                    stride = 2;
                ...
                otherwise
                    error('Don''t know how to interpret cal.describe.nGammaBands = ''%s''. ',cal.describe.nGammaBands); 
            end
            cal.describe.gamma.gammaBands = 1:stride:cal.describe.numWavelengthBands;
            cal.describe.nGammaBands = numel(cal.describe.gamma.gammaBands);
        else
