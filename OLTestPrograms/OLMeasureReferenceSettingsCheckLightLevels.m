...
Scal = cal.describe.S;
radianceWattsPerM2SrCal = mean(cal.raw.fullOnMeas, 2);
...
fullOnMeas = SplineSpd(Scal,cal.raw.fullOnMeas,S);
fullOnXYZ = mean(T_xyz*fullOnMeas,2);

darkMeas = SplineSpd(Scal,cal.raw.darkMeas,S);
darkXYZ = mean(T_xyz*darkMeas,2);

halfOnMeas = SplineSpd(Scal,cal.raw.halfOnMeas,S);
halfOnXYZ = mean(T_xyz*halfOnMeas,2);
fprintf('Full on luminance: %0.1f cd/m2, Half on luminance: %0.1f cd/m2, dark luminance: %0.1f cd/m2\n',fullOnXYZ(2),halfOnXYZ(2),darkXYZ(2));
...
curDir = pwd;
cd(plotFolder);
%savefig(plotRoot,fig2,'pdf');
savefig(plotRoot,fig2,'png');
cd(curDir);