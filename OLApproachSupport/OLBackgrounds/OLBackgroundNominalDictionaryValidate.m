function valid = OLBackgroundNominalDictionaryValidate(entry)
%OLBACKGROUNDDICTIONARYVALIDATE Validate parameters and add this
%entry to dictionary

valid = false;

% Get all the expected field names for this type
allFieldNames = fieldnames(OLBackgroundNominalDictionaryDefaults(entry.type));
...
switch (entry.type)
    case 'optimized'
        ...
        assert((isfield(entry, 'pegBackground')              && islogical(entry.pegBackground)),          sprintf('params.pegBackground does not exist or it does not contain a boolean value.'));
        ...
        assert((isfield(entry, 'backgroundObserverAge')      && isnumeric(entry.pupilDiameterMm)),        sprintf('params.backgroundObserverAge does not exist or it does not contain a number.'));
