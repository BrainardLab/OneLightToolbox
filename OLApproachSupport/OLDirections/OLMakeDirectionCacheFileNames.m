...
%   setting it to 4/6 and letting the user know big time.
    ...
    for k = 1:numel(protocolParams.directionNames)
        directionCacheFileNames{k} = sprintf('Direction_%s_%d_%d_%d',protocolParams.directionNames{k}, round(10*protocolParams.fieldSizeDegrees),round(10*protocolParams.pupilDiameterMm),round(1000*protocolParams.directionModulationContrast));
    end
end